...
class AnalyticsRulesV1Test extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->ruleUpsertResponse = $this->client()->analyticsV1->rules()->upsert($this->ruleName, $this->ruleConfiguration);
    }
    ...
    protected function tearDown(): void
    {
        if (!$this->isV30OrAbove()) {
            try {
                $rules = $this->client()->analyticsV1->rules()->retrieve();
                if (is_array($rules) && isset($rules['rules'])) {
                    foreach ($rules['rules'] as $rule) {
                        $this->client()->analyticsV1->rules()->{$rule['name']}->delete();
                    }
                }
            } catch (Exception $e) {
            }
    ...
    {
        $returnData = $this->client()->analyticsV1->rules()[$this->ruleName]->retrieve();
        $this->assertEquals($returnData['name'], $this->ruleName);
    }

    public function testCanDeleteARule(): void
    {
        $returnData = $this->client()->analyticsV1->rules()[$this->ruleName]->delete();
        $this->assertEquals($returnData['name'], $this->ruleName);

        $this->expectException(ObjectNotFound::class);
        $this->client()->analyticsV1->rules()[$this->ruleName]->retrieve();
    }

    public function testCanRetrieveAllRules(): void
    {
        $returnData = $this->client()->analyticsV1->rules()->retrieve();
